...
<body>
    ...
    <div class="mb-3">
        ...
        <input type="file" class="form-control visually-hidden" id="item_image" name="item_image[]" accept="image/*"
            onchange="previewImage(event)" required multiple>
...
function resizeImage($file, $maxWidth, $maxHeight, $targetFile)
{
    list($originalWidth, $originalHeight, $imageType) = getimagesize($file);

    // Calculate resize ratio
    $resizeRatio = min($maxWidth / $originalWidth, $maxHeight / $originalHeight);
    ...
    // Resize and copy the original image onto the new image
    imagecopyresampled($resizedImage, $sourceImage, 0, 0, 0, 0, $newWidth, $newHeight, $originalWidth, $originalHeight);

    // Save the resized image to the target file
    switch ($imageType) {
        case IMAGETYPE_JPEG:
            imagejpeg($resizedImage, $targetFile, 90); // Adjust quality as needed (0-100)
            break;
        case IMAGETYPE_PNG:
            imagepng($resizedImage, $targetFile, 9); // Adjust compression level as needed (0-9)
            break;
        case IMAGETYPE_GIF:
    ...
    return $targetFile; // Return the path to the resized image
}
